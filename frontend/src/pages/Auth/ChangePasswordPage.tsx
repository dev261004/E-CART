...
import { changePasswordSchema, type ChangePasswordFormInput } from "@/validation/changePasswordSchema";
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (!parsed.success) {
    ...
    parsed.error.issues.forEach((issue) => {
      const path = issue.path?.[0] as keyof ChangePasswordFormInput | undefined;
  ...
  try {
    ...
    // ✅ token & watcher suppression already handled inside authService.changePassword
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 via-indigo-50 to-slate-100 px-4 py-8">
            <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* LEFT PANEL – security illustration */}
                    ...
                    <div className="relative z-10 space-y-6">
                        ...
                        <ul className="mt-4 space-y-2 text-sm text-indigo-100">
                            <li className="flex items-start gap-2">
                                ...
                                <span>Auto logout on other devices after password change.</span>
                            </li>
                            <li className="flex items-start gap-2">
                                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-emerald-300" />
                                <span>Strong password requirements enforced for your safety.</span>
                            </li>
                            <li className="flex items-start gap-2">
                                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-emerald-300" />
                                <span>Works seamlessly for Admin, Vendor, and Buyer accounts.</span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-xl border border-gray-100 p-6 sm:p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-lg border px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${getError("currentPassword") ? "border-rose-500" : "border-gray-200"
                                        }`}
                                />
                            ...
                            {getError("currentPassword") && (
                                <p className="mt-1 text-xs text-rose-600">{getError("currentPassword")}</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-lg border px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${getError("newPassword") ? "border-rose-500" : "border-gray-200"
                                        }`}
                                />
                            ...
                            {getError("newPassword") && (
                                <p className="mt-1 text-xs text-rose-600">{getError("newPassword")}</p>
                            )}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className={`mt-1 block w-full rounded-lg border px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${getError("confirmNewPassword") ? "border-rose-500" : "border-gray-200"
                                        }`}
                                />
                        ...
                        <button
                            type="submit"
                            disabled={submitting}
                            className={`mt-2 w-full inline-flex justify-center items-center px-4 py-2 rounded-full text-sm font-medium text-white shadow ${submitting
